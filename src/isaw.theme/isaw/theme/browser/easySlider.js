    ...
    this.each(function() {
      ...
      if(options.navigation_type != "No Buttons"){
        ...
        if(options.navigation_type == "Navigation Buttons"){
          ...
          html += ' <button id="leftb" class="leftright">';
          html +='<img id="prev-bt" src="++resource++prevII.gif" title="" alt="' + previous_picture_alt + '" /></button>';
          count = 1;
          for(i=0; i < total_slides+1; i++){ 
            ...
            if(add){
              html += "<button class=\"nav-button\" id=\"" + options.goToButtonId + i + "\">" + count + "</button> ";
              count++;
            }
          }
          html += ' <button id="rightb" class="leftright">';
          html += '<img id="next-bt" src="++resource++nextII.gif" alt="' + next_picture_alt + '"  /></button>';
          html += '<button id="pauseplay"><img src="++resource++sliderpause.png" alt="pause"/></button>';
          html += "</div>";
          ...
          for(i=0; i < total_slides+1; i++){
            ...
            if(add){
              html += "<button class=\"nav-button\" id=\"" + options.goToButtonId + i + "\">" + "•" + "</button> ";
              count++;
            }
          }
          if(options.navigation_type != "Bullets Only")  {
            html += "</div>";
            html += '<button class=\"easybutton\" id="'+ options.prevId +'">&nbsp</button>';
            html += '<button class=\"easybutton\" id="'+ options.nextId +'">&nbsp</button>';
          }
        } else if(options.navigation_type.indexOf("Big") >= 0){
          html += ' <button id="'+ options.prevId +'">&nbsp</button>';
          html += ' <button id="'+ options.nextId +'">&nbsp</button>';
      ...
      $("button","#"+options.nextId, container).click(function(){    
        schedule(function(){ animate("next",true, container); }, true);
      });
      $("button","#"+options.prevId, container).click(function(){    
        schedule(function(){ animate("prev",true, container); }, true);
      }); 
      $("button#leftb", container).click(function(){    
        schedule(function(){ animate("prev",true, container); }, true);
      });
      $("button#rightb", container).click(function(){   
        schedule(function(){ animate("next",true, container); }, true);
      });
      $("button","#"+options.firstId, container).click(function(){   
        schedule(function(){ animate("first",true, container); }, true);
      });       
      $("button","#"+options.lastId, container).click(function(){    
        schedule(function(){ animate("last",true, container); }, true);
      }); 
      $("div#easySlider-goToButtons button:not(.leftright, #pauseplay)", container).click(function(){
      ...
      update_goToButtons($("div#easySlider-goToButtons button:not(.leftright, #pauseplay)", container));
      ...
      function animate(dir, clicked, container){
        ...
        if((options.navigation_type == "Navigation Buttons" || options.navigation_type == "Navigation Bullets") && $.inArray(current_slide, skipped_navigation) == -1){
          var btns = $("div#easySlider-goToButtons button:not(.leftright, #pauseplay)", container);
        ...
        if(!options.continuous){          
          if(current_slide==total_slides){
            $("button","#"+options.nextId, container).hide();
            $("button","#"+options.lastId, container).hide();
          } else {
            $("button","#"+options.nextId, container).show();
            $("button","#"+options.lastId, container).show();          
          }
          if(current_slide === 0){
            $("button","#"+options.prevId, container).hide();
            $("button","#"+options.firstId, container).hide();
          } else {
            $("button","#"+options.prevId, container).show();
            $("button","#"+options.firstId, container).show();
          }
        }
      ...
      function set_timer(duration){
        ...
        $('button#pauseplay img', container).attr('src', '++resource++sliderpause.png');
      }
      ...
      function clear_timer(){
        ...
        $('button#pauseplay img', container).attr('src', '++resource++sliderplay.png');
      }
      ...
      if(timer_activated){
        ...
        $('button#pauseplay img').attr('src', '++resource++sliderpause.png');
      }else{
        $('button#pauseplay', container).addClass('play');
        $('button#pauseplay img').attr('src', '++resource++sliderplay.png');
      }
