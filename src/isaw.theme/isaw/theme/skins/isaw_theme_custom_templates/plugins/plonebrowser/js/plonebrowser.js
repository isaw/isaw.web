...
BrowserDialog.prototype.init = function () {
    ...
    if (this.is_link_plugin === true) {
        ...
        if (selected_node.get(0).tagName && selected_node.get(0).tagName.toUpperCase() === 'IMG') {
            ...
            // get alt text from image attributes
            jq('#alt', document).val(selected_node.get(0).alt);
            jq('#caption', document).val(selected_node.get(0).title);
...
BrowserDialog.prototype.insertImage = function () {
    ...
    if (active_panel === "#external") {
        ...
        // Image captioning
        if (this.editor.settings.allow_captioned_images && jq('#caption', document).val() && jq.inArray('captioned', classes) == -1) {
    ...
    jq.extend(attrs, {
        'src' : href,
        'alt' : jq('#alt', document).val(),
        'title' : jq('#caption', document).val(),
        'class' : classes.join(' ')
    });
...
BrowserDialog.prototype.setDetails = function (url) {
    ...
    jq.ajax({
        ...
        'success': function (data) {
            ...
            jq('#descriptiondetails', document).text(data.description);
            if (self.is_link_plugin === true) {
