...
def createHomePage(context):
    ...
    page_id = 'isaw-home'
    if page_id not in context.objectIds():
        context.invokeFactory('Document', page_id)
        home_page = context[page_id]
        ...
        publish(home_page)
        set_default_page(context, page_id)
